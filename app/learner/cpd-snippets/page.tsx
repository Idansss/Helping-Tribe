...
export default function LearnerCpdSnippetsPage() {
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-slate-600 mt-1">
          Quick-reference cards to support your Continuing Professional Development.
        </p>
      </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-16 text-sm text-slate-500">
          {snippets.length === 0 ? 'No CPD snippets have been added yet. Check back later.' : 'No snippets match your search.'}
        </div>
      ) : (
          ...
          {grouped.map((group) => (
            <div key={group.value} className="space-y-3">
              <div className="flex items-center gap-2">
                <Badge variant="outline" className={`text-xs ${categoryColors[group.value] ?? categoryColors.general}`}>
                  {group.label}
                </Badge>
                <span className="text-xs text-slate-400">{group.items.length} snippet{group.items.length !== 1 ? 's' : ''}</span>
              </div>
