...
export function HeroSection() {
  return (
    <section className="relative overflow-hidden bg-[#0b1320] pt-[calc(var(--nav-height)+2rem)] text-white md:pt-[calc(var(--nav-height)+2.5rem)]">
      ...
      <div className="relative mx-auto grid max-w-7xl gap-10 px-[var(--page-gutter)] pb-16 lg:grid-cols-[1.02fr_0.98fr] lg:items-start lg:gap-12 lg:pb-24">
        <div className="order-1 max-w-3xl py-2 lg:order-none lg:py-4" data-reveal="heading">
          ...
          <ul className="mt-10 hidden gap-3 border-t border-white/12 pt-6 sm:grid sm:grid-cols-3 lg:grid" data-reveal>
        ...
        <div className="order-2 lg:order-none" data-reveal="scale">
          <GrowthOrbitVisual />
        </div>
        <ul className="order-3 mt-2 grid gap-3 border-t border-white/12 pt-6 sm:hidden" data-reveal>
          {facts.map(({ icon: Icon, label }) => (
            <li key={label} className="flex items-center gap-2 text-sm text-slate-300">
              <Icon className="size-4 shrink-0 text-[#d8bd7a]" aria-hidden="true" />
              <span>{label}</span>
